...
"""Finding optimal configuration of 8 points in a square that minimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    Returns:
        A: np.array of shape (8, 2) with coordinates of points in the square.
        x and y coordinates are in the range [0, 1].
    """

    def discrepancy_loss(x):
        points = x.reshape(8, 2)
        return star_discrepancy(points)

    # Initial guess: Use a Sobol sequence for better initial distribution
    from scipy.stats import qmc
    sampler = qmc.Sobol(d=2, scramble=True)  # Use Sobol sequence and scramble
    initial_guess = sampler.random(n=8)
    initial_guess = initial_guess.flatten()

    # Bounds for each coordinate: 0 <= x, y <= 1
    bounds = [(0.0, 1.0)] * 16

    # Optimization using SLSQP, increase maxiter and consider different tolerances
    # Also, try different initial points by running the optimization multiple times
    best_result = None
    best_loss = float('inf')
    
    for _ in range(25):  # Run optimization 25 times with different starting points
        initial_guess_temp = sampler.random(n=8).flatten()
        result = minimize(discrepancy_loss, initial_guess_temp, method='SLSQP', bounds=bounds, options={'maxiter': 4000, 'ftol': 1e-14, 'iprint': 0}) # Increased maxiter, ftol
    ...
    if best_result is not None:
        A = best_result.x.reshape(8, 2)
        return A
    else:
        print("Optimization failed, returning initial guess.")
        return initial_guess.reshape(8, 2)
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=8"""
    A = construct_star()
    return A
