...
"""Finding optimal configuration of 19 points in a square that mimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function constructs 19 points and optimizes their position using scipy.optimize.minimize.
    Returns:
        A: np.array of shape (19, 2) with coordinates of points in the square.
    ...
    # Initial guess: Optimized stratified sampling with jitter, smaller jitter
    initial_points = np.zeros((N, 2))
    for i in range(N):
        initial_points[i, 0] = (i + np.random.rand() * 0.2 + 0.4) / N  # Centered in each interval, with jitter
        initial_points[i, 1] = ((i * (np.sqrt(5) - 1) / 2 + (np.random.rand() * 0.2 + 0.4)/N) % 1) # Golden ratio sampling, centered, with jitter
    ...
    # Increased ftol and maxiter for potentially better result, and adjust eps
    result = minimize(discrepancy_wrapper, initial_guess, method='SLSQP', bounds=bounds, options={'maxiter': 5000, 'ftol': 1e-15, 'eps': 1e-8})
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=19"""
    A = construct_star()
    return A
