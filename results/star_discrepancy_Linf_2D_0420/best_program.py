...
"""Finding optimal configuration of 420 points in a square that mimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function constructs 420 points that are evenly distributed.
    The points are represented in coordinates (x, y) in the square
    x and y are in the range [0, 1].
    Returns:
        A: np.array of shape (420, 2) with coordinates of points in the square.
        x and y coordinates are in the range [0, 1].
    """
    N = 420

    def discrepancy_loss(x):
        """Calculates the star discrepancy for a given set of offsets."""
        points = np.zeros((N, 2))
        points[:, 0] = (np.arange(N) + x[0]) / N
        points[:, 1] = (np.mod(np.arange(N) * ((1 + np.sqrt(5)) / 2), 1.0) + x[1]) % 1.0
        return star_discrepancy(points)

    # Initial guess for the offsets
    initial_guess = [0.0, 0.0]

    # Bounds for the offsets (they must be between 0 and 1)
    bounds = [(0.0, 1.0), (0.0, 1.0)]

    # Optimization using SLSQP with tighter tolerance and increased maxiter
    # Using a different initial guess and increasing maxiter further
    # Using a better initial guess and increasing maxiter further
    result = minimize(discrepancy_loss, initial_guess, method='SLSQP', bounds=bounds, options={'ftol': 1e-8, 'maxiter': 5000})

    # Extract optimized offsets
    optimized_offset_x = result.x[0]
    optimized_offset_y = result.x[1]

    A = np.zeros((N, 2))
    A[:, 0] = (np.arange(N) + optimized_offset_x) / N
    A[:, 1] = (np.mod(np.arange(N) * ((1 + np.sqrt(5)) / 2), 1.0) + optimized_offset_y) % 1.0
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=420"""
    A = construct_star()
    return A
