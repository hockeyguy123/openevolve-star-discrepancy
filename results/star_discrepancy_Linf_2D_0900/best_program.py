...
"""Finding optimal configuration of 900 points in a square that mimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function constructs 900 points that are evenly distributed and maximally distant from each other.
    The points are represented in coordinates (x, y) in the square
    x and y are in the range [0, 1].
    Returns:
        A: np.array of shape (900, 2) with coordinates of points in the square.
        x and y coordinates are in the range [0, 1].
    """
    N = 900
    A = np.zeros((N, 2))
    
    # Optimized stratified sampling with a golden ratio sequence for both x and y
    golden_ratio = (1 + math.sqrt(5)) / 2
    
    # Precompute the offsets
    x_offsets = (golden_ratio / (2 * N))
    
    for i in range(N):
        # X-coordinate: Stratified sampling with golden ratio offset
        A[i, 0] = ((i / N) + x_offsets) % 1

        # Y-coordinate: Golden ratio sequence with a different offset
        A[i, 1] = ((i * golden_ratio + 0.5) % 1) # Added 0.5 offset
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=900"""
    A = construct_star()
    return A
