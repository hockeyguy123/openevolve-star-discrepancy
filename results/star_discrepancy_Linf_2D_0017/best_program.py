...
"""Finding optimal configuration of 17 points in a square that mimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function constructs 17 points that are evenly distributed and maximally distant from each other.
    The points are represented in coordinates (x, y) in the square
    x and y are in the range [0, 1].
    Returns:
        A: np.array of shape (17, 2) with coordinates of points in the square.
    ...
    # Initial guess: Optimized initialization using a Sobol sequence and jittering
    rng = np.random.default_rng(seed=42)  # Consistent random seed
    initial_guess = np.zeros((N, 2))
    
    # Use a Sobol sequence-like initialization for both dimensions, more aggressive jittering
    sobol_x = np.array([(2*i + 1) / (2*N) for i in range(N)])
    sobol_y = np.array([((i * 0.618034) % 1) for i in range(N)])
    
    initial_guess[:, 0] = sobol_x + rng.uniform(-0.3/N, 0.3/N, size=N)
    initial_guess[:, 1] = sobol_y + rng.uniform(-0.3/N, 0.3/N, size=N)

    initial_guess = np.clip(initial_guess, 0.0, 1.0).flatten()

    # Define bounds for each coordinate (0 <= x, y <= 1)
    bounds = [(0.0, 1.0) for _ in range(2 * N)]

    # Use SLSQP optimization with increased maxiter and ftol, and a more robust approach to handling constraints.
    result = minimize(discrepancy_to_minimize, initial_guess, method='SLSQP', bounds=bounds, options={'maxiter': 100000, 'ftol': 1e-16})
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=17"""
    A = construct_star()
    return A
