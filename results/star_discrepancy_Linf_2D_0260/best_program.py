...
"""Finding optimal configuration of 260 points in a square that mimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function constructs 260 points and then uses scipy.optimize.minimize
    to fine-tune their positions.
    Returns:
        A: np.array of shape (260, 2) with coordinates of points in the square.
        x and y coordinates are in the range [0, 1].
    """
    N = 260
    initial_points = np.zeros((N, 2))

    # Initial point distribution using a combination of stratification and a modified golden ratio sequence.
    for i in range(N):
        initial_points[i, 0] = (i + 0.5) / N  # Stratified x-coordinates
        initial_points[i, 1] = (0.5 + (i * 0.61803398875) % 1) % 1  # Golden ratio sequence for y-coordinates
    ...
    initial_points_flat = initial_points.flatten()

    # Define bounds for each point to ensure they stay within [0, 1] x [0, 1]
    bounds = [(0.0, 1.0) for _ in range(2 * N)]

    # Use L-BFGS-B optimizer, which is generally more efficient for bound-constrained problems
    result = minimize(objective_function, initial_points_flat, method='L-BFGS-B', bounds=bounds, options={'maxiter': 300, 'ftol': 1e-7})

    # Reshape the optimized flat array back into the (N, 2) array
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=260"""
    A = construct_star()
    return A
