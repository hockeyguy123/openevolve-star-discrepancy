...
"""Finding optimal configuration of 20 points in a square that mimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function constructs 20 points that are evenly distributed.
    The points are represented in coordinates (x, y) in the square
    x and y are in the range [0, 1].
    Returns:
        A: np.array of shape (20, 2) with coordinates of points in the square.
    ...
    # Initial guess: Centered grid with a small perturbation
    initial_guess = np.zeros((N, 2))
    for i in range(N):
        initial_guess[i, 0] = (i + 0.5) / N
        initial_guess[i, 1] = ((i * 0.381966) + 0.5) % 1
    
    # Perturb the initial guess, but with a smaller perturbation
    initial_guess += np.random.normal(0, 0.005, size=(N, 2))
    ...
    # Optimization using SLSQP, increased maxiter and ftol
    # Using a different initialization strategy for SLSQP
    result = minimize(discrepancy_to_minimize, initial_guess, method='SLSQP', bounds=bounds, options={'maxiter': 5000, 'ftol': 1e-14})
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=20"""
    A = construct_star()
    return A
