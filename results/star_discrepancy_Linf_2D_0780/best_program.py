...
"""Finding optimal configuration of 780 points in a square that mimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function constructs 780 points that are evenly distributed.
    The points are represented in coordinates (x, y) in the square
    x and y are in the range [0, 1].
    Returns:
        A: np.array of shape (780, 2) with coordinates of points in the square.
        x and y coordinates are in the range [0, 1].
    """
    N = 780
    A = np.zeros((N, 2))

    # Optimized Latin hypercube sampling with golden ratio and a tent map
    for i in range(N):
        # Latin hypercube for x
        A[i, 0] = (i + 0.5) / N

        # Tent map with golden ratio for y
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=780"""
    A = construct_star()
    return A
