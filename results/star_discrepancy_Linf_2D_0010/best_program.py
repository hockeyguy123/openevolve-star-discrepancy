...
"""Finding optimal configuration of 10 points in a square that mimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function constructs 10 points that are evenly distributed in the square
    x and y are in the range [0, 1].
    Returns:
        A: np.array of shape (10, 2) with coordinates of points in the square.
    ...
    # Bounds for each point (0 <= x, y <= 1)
    bounds = [(0.0, 1.0) for _ in range(2 * N)]

    # SLSQP optimization - more robust initialization and iterations
    best_points = None
    best_discrepancy = float('inf')

    # More attempts with different initializations
    for _ in range(40):  # Increased attempts to explore the space better
        # Initial guess - Sobol sequence, increased scrambling
        sampler = qmc.Sobol(d=2, scramble=True)
        initial_points = sampler.random_base2(m=int(np.ceil(np.log2(N))))[:N]

        # Latin Hypercube Sampling for initial points
        # sampler = qmc.LatinHypercube(d=2)
        # initial_points = sampler.random(n=N)

        # Perturb the initial points slightly to avoid getting stuck in local minima
        initial_points += np.random.normal(0, 0.005, size=initial_points.shape)
        initial_points = np.clip(initial_points, 0.0, 1.0)

        # Use a different initial point for each attempt
        x0 = initial_points.flatten()
        
        result = minimize(discrepancy_wrapper, x0, 
                          method='SLSQP', bounds=bounds, options={'maxiter': 5000, 'ftol': 1e-12}) # Increased iterations, tightened tolerance
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=10"""
    A = construct_star()
    return A
