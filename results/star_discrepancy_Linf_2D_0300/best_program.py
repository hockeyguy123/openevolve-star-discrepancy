...
"""Finding optimal configuration of 300 points in a square that mimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function constructs 300 points that are evenly distributed, then optimizes them.
    The points are represented in coordinates (x, y) in the square
    x and y are in the range [0, 1].
    Returns:
        A: np.array of shape (300, 2) with coordinates of points in the square.
    ...
    for i in range(N):
        ...
        initial_points[i, 1] = (math.fmod((i * 0.61803398875) + 0.5, 1))  # Using the golden ratio and fmod for better numerical stability
    ...
    # Use SLSQP optimizer
    result = minimize(discrepancy_wrapper, x0, method='SLSQP', bounds=bounds, options={'maxiter': 50})
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=300"""
    A = construct_star()
    return A
