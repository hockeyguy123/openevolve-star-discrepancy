...
"""Finding optimal configuration of 500 points in a square that minimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function constructs 500 points that are evenly distributed.
    The points are represented in coordinates (x, y) in the square
    x and y are in the range [0, 1].
    Returns:
        A: np.array of shape (500, 2) with coordinates of points in the square.
        x and y coordinates are in the range [0, 1].
    """
    N = 500
    A = np.zeros((N, 2))

    # Use a combination of Golden Ratio and a linear sequence for better distribution.
    golden_ratio = 0.61803398875
    for i in range(N):
        A[i, 0] = (i + 0.5) / N
        A[i, 1] = (golden_ratio * (i + 0.5)) % 1.0
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=500"""
    A = construct_star()
    return A
