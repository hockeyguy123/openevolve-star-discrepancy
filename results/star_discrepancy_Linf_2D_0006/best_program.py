...
"""Finding optimal configuration of 6 points in a square that mimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function constructs 6 points that are evenly distributed and maximally distant from each other.
    The points are represented in coordinates (x, y) in the square
    x and y are in the range [0, 1].
    Returns:
        A: np.array of shape (6, 2) with coordinates of points in the square.
    ...
    # Initial guess (more dispersed initial points and a bit of structure)
    initial_guess = np.array([
        ...
        0.1, 0.9,
        0.9, 0.9,
        0.5, 0.3  # Added a point slightly off-center
    ])
    
    # Bounds for each coordinate (0 <= x, y <= 1)
    bounds = [(0.0, 1.0)] * (N * 2)
    
    # Optimization using SLSQP with tighter tolerances and more iterations
    constraints = []
    # Add a constraint to prevent points from being too close to each other
    for i in range(N):
        for j in range(i + 1, N):
            def distance_constraint(x, i=i, j=j):
                points = x.reshape(N, 2)
                dist = np.linalg.norm(points[i] - points[j])
                return dist - 0.1  # Ensure a minimum distance of 0.1 between points
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=6"""
    A = construct_star()
    return A
