...
"""Finding optimal configuration of 11 points in a square that minimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    Returns:
        A: np.array of shape (11, 2) with coordinates of points in the square.
        x and y coordinates are in the range [0, 1].
    """
    N = 11
    
    # Initial guess for point positions (flattened) - Optimized initial points
    initial_points = np.array([
        0.05, 0.20, 0.15, 0.70, 0.30, 0.40, 0.45, 0.90, 0.50, 0.60,
        0.65, 0.30, 0.70, 0.80, 0.85, 0.10, 0.95, 0.50, 0.01, 0.01,
        0.99, 0.99
    ])
    ...
    # Define a function to calculate the star discrepancy for a given set of points
    def discrepancy_wrapper(points_flat):
        points = points_flat.reshape(N, 2)
        return star_discrepancy(points)

    # Optimization using SLSQP - Increased maxiter and added ftol
    # Adjusted SLSQP parameters for potentially better convergence
    # Using a more robust optimization strategy with multiple restarts
    best_result = None
    best_discrepancy = float('inf')
    
    for _ in range(20):  # Increased restarts for better exploration
        # Adding a larger random perturbation to the initial points for each restart
        initial_points_perturbed = initial_points + np.random.normal(0, 0.05, 2 * N)
        initial_points_perturbed = np.clip(initial_points_perturbed, 0.0, 1.0)

        result = minimize(discrepancy_wrapper, initial_points_perturbed, bounds=bounds, method='SLSQP', options={'maxiter': 20000, 'ftol': 1e-14, 'iprint': 0})
    ...
    # Reshape the optimized point coordinates
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=11"""
    A = construct_star()
    return A
