...
"""Finding optimal configuration of 220 points in a square that mimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function constructs 220 points that are evenly distributed and then refines their positions.

    Returns:
        A: np.array of shape (220, 2) with coordinates of points in the square.
        x and y coordinates are in the range [0, 1].
    """
    N = 220
    
    # Initialize points with a Sobol sequence-like approach for better initial distribution
    p = np.arange(1, N + 1)
    initial_points = np.zeros((N, 2))
    initial_points[:, 0] = (2 * p - 1) / (2 * N)  # Centered Halton sequence for x
    initial_points[:, 1] = ((p * 0.61803398875) % 1.0) # Golden ratio sequence for y

    def objective_function(x):
        points = x.reshape(N, 2)
        return star_discrepancy(points)

    # Flatten the initial points for the optimization
    x0 = initial_points.flatten()

    # Define bounds for each point to stay within [0, 1]
    bounds = [(0.0, 1.0) for _ in range(2 * N)]

    # Use SLSQP optimizer with increased iterations and tighter tolerance
    # Increased maxiter and ftol for potentially better convergence
    result = minimize(objective_function, x0, method='SLSQP', bounds=bounds, options={'maxiter': 500, 'ftol': 1e-8})
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=220"""
    A = construct_star()
    return A
