...
"""Finding optimal configuration of 50 points in a square that mimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function constructs 50 points.
    The points are represented in coordinates (x, y) in the square
    x and y are in the range [0, 1].
    Returns:
        A: np.array of shape (50, 2) with coordinates of points in the square.
        x and y coordinates are in the range [0, 1].
    """
    N = 50
    
    # Initial guess: Centered within each interval and golden ratio
    initial_points = np.zeros((N, 2))
    for i in range(N):
        initial_points[i, 0] = (i + 0.5) / N
        initial_points[i, 1] = ((i * 0.61803398875) % 1)
    ...
    # Define bounds for each point (0 <= x, y <= 1)
    bounds = [(0.0, 1.0)] * (2 * N)

    # Optimization using SLSQP - Increased maxiter and ftol, and adjusted eps
    # Using a more robust initialization and increasing the number of iterations
    # Adjusted tolerances and maxiter for potentially better convergence
    # Added a small perturbation to the initial points to avoid local minima
    
    # More refined perturbation, ensuring points stay within bounds
    perturbation = np.random.normal(0, 0.001, size=x0.shape)
    x0 = np.clip(x0 + perturbation, 0.0, 1.0)
    
    # Implementing a grid search to improve initialization
    grid_size = 5
    best_points = None
    ...
    for i in range(grid_size):
        for j in range(grid_size):
            initial_points_grid = np.zeros((N, 2))
            for k in range(N):
                initial_points_grid[k, 0] = ((k + 0.5) / N + i/grid_size/N/2) % 1
                initial_points_grid[k, 1] = (((k * 0.61803398875) % 1) + j/grid_size/N/2) % 1
            
            x0_grid = initial_points_grid.flatten()
            
            result = minimize(discrepancy_wrapper, x0_grid, method='SLSQP', bounds=bounds,
                              options={'maxiter': 5000, 'ftol': 1e-10, 'eps': 1e-08})
            
            points_grid = result.x.reshape(N, 2)
            discrepancy = star_discrepancy(points_grid)
            
            if discrepancy < best_discrepancy:
                best_discrepancy = discrepancy
                best_points = result.x
                
    # Use best points from grid search as initial point for final optimization
    if best_points is not None:
        result = minimize(discrepancy_wrapper, best_points, method='SLSQP', bounds=bounds,
                          options={'maxiter': 10000, 'ftol': 1e-12, 'eps': 1e-08})
        A = result.x.reshape(N, 2)
    else:
        A = x0.reshape(N, 2)

    # Reshape the optimized result back into points
    #A = result.x.reshape(N, 2)
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=50"""
    A = construct_star()
    return A
    ...
    for i_point in range(N_arg): # Iterate through each point
        ...
        for k_dim in range(D_arg): # Iterate through each dimension for the current point
            if points_X_arg[i_point, k_dim] > y_corner_arg[k_dim]: # Point is outside this dimension
                ...
                break
            elif abs(points_X_arg[i_point, k_dim] - y_corner_arg[k_dim]) < 1e-15:
