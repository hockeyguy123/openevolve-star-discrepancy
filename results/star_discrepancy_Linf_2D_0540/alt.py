...
def construct_star(N: int) -> np.ndarray:
    """
    Find the optimal configuration of points in a square [0, 1] x [0, 1]
    to minimize the L-infinity star discrepancy.
    This function constructs 540 points that are evenly distributed and maximally distant from each other.
    ...
    for i in range(N):
        A[i, 0] = (i + shift) / N  # Stratified sampling in x
        # Use a modified golden ratio for y-coordinates, shifted and scaled
        A[i, 1] = ((i * golden_ratio) % 1 + (shift / N)) % 1 # Golden ratio for y-coordinates, shifted.
    return A

    # Sobol sequence
    # import scipy.stats.qmc
    # qmc_engine = qmc.Sobol(d=2, seed=123)
    # A = qmc_engine.random(n=N)

    # Halton sequence
    # import scipy.stats.qmc
    # qmc_engine = qmc.Halton(d=2, scramble=True, seed=123)
    # A = qmc_engine.random(n=N)

    # Latin Hypercube sequence
    # import scipy.stats.qmc
    # qmc_engine = qmc.LatinHypercube(d=2, seed=123)
    # A = qmc_engine.random(n=N)

    # Hammersely sequence
    # def radical_inverse_base2(n):
    #     inverse = 0.0
    #     power_of_2_denominator = 0.5  # This is 1/2, for the first digit after the decimal point
        
    #     while n > 0:
    #         if n % 2 == 1:  # If the last bit of n is 1
    #             inverse += power_of_2_denominator
    #         n //= 2  # Integer division to remove the last bit
    #         power_of_2_denominator /= 2
            
    #     return inverse
    
    # A = np.zeros((N, 2)) # N rows, 2 columns (x, y)
    
    # for i in range(N):
    #     A[i, 0] = (i + 0.5) / N
    #     A[i, 1] = radical_inverse_base2(i)
...
if __name__ == "__main__":
    ...
    numbers += [140, 180, 220, 260, 300, 340, 380, 420, 460, 500, 540, 580, 620, 660, 700, 740, 780, 820, 860, 900, 940, 980, 1020]
    for i in numbers:
        points = construct_star(i)
        discrepancy_value = star_discrepancy(points)    
        print(f"Star Discrepancy Value for {i} points:", discrepancy_value)
