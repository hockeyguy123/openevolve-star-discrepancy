...
"""Finding optimal configuration of 5 points in a square that minimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function constructs 5 points that are strategically distributed.
    Returns:
        A: np.array of shape (5, 2) with coordinates of points in the square.
        x and y coordinates are in the range [0, 1].
    """
    # Initial guess for the point positions. More structured initial guess.
    ...
    # Define the bounds for each coordinate (0 <= x, y <= 1)
    bounds = [(0.0, 1.0)] * 10  # 5 points * 2 coordinates

    # Define the objective function to minimize (negative of the score)
    def objective_function(x):
        points = x.reshape(5, 2)
        return -score_star(points)

    # Constraint: No two points can be too close to each other.  Relaxed distance constraint.
    def distance_constraint(x):
        ...
        return min_distance - 0.025 # Ensure a minimum distance of 0.025

    constraints = ({'type': 'ineq', 'fun': distance_constraint})

    # Use the SLSQP optimization algorithm to find the optimal point positions
    # Using a more robust optimization strategy with multiple restarts and a smaller initial perturbation.
    best_result = None
    best_score = -np.inf

    for _ in range(15):  # Reduced restarts slightly
        initial_guess_perturbed = initial_guess + np.random.normal(0, 0.005, size=initial_guess.shape)  # Slightly smaller perturbation
        initial_guess_perturbed = np.clip(initial_guess_perturbed, 0.0, 1.0)
        result = minimize(objective_function, initial_guess_perturbed, method='SLSQP', bounds=bounds, constraints=constraints, options={'maxiter': 3000, 'ftol': 1e-10})  # Adjusted maxiter and tolerance

        if -result.fun > best_score:
            best_score = -result.fun
            best_result = result

    # Reshape the optimized point positions into a 5x2 array
    A = best_result.x.reshape(5, 2)
    return A
