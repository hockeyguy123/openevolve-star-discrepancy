...
"""Finding optimal configuration of 460 points in a square that minimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function constructs 460 points that are evenly distributed and maximally distant from each other.
    The points are represented in coordinates (x, y) in the square
    x and y are in the range [0, 1].
    Returns:
        A: np.array of shape (460, 2) with coordinates of points in the square.
        x and y coordinates are in the range [0, 1].
    """
    N = 460
    A = np.zeros((N, 2))

    # Optimized stratified sampling with golden ratio and slight offset
    phi = (1 + np.sqrt(5)) / 2
    for i in range(N):
        A[i, 0] = (i + 0.5) / N
        A[i, 1] = ((i * phi + 0.5*phi) % 1) # Adding extra offset

    # Optimized Tent Map transformation
    y = A[:, 1]
    y = np.where(y < 0.5, 2 * y, 2 * (1 - y))
    A[:, 1] = y

    # Optimized Perturbation with smaller magnitude and different distribution
    rng = np.random.default_rng(seed=42)
    # Use a triangular distribution for perturbation.  This tends to concentrate points
    # near the stratified location, but allows for some variation.
    perturbation = rng.triangular(-0.000005, 0, 0.000005, size=(N, 2)) # Reduced magnitude
    A += perturbation
    A = np.clip(A, 0.0, 1.0)

    # Refine the distribution by adding a small adjustment to the y-coordinates
    # based on the x-coordinate. This aims to reduce clustering.
    adjustment_factor = 0.000005 # Reduced factor
    A[:, 1] += adjustment_factor * (A[:, 0] - 0.5)
    A = np.clip(A, 0.0, 1.0)

    # Introduce a small rotation
    theta = 0.0001 # Small rotation angle
    rotation_matrix = np.array([[np.cos(theta), -np.sin(theta)], [np.sin(theta), np.cos(theta)]])
    center = np.array([0.5, 0.5])
    A = (A - center) @ rotation_matrix + center
    A = np.clip(A, 0.0, 1.0)
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=460"""
    A = construct_star()
    return A
