...
"""Finding optimal configuration of 13 points in a square that mimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function constructs 13 points that are evenly distributed.
    Returns:
        A: np.array of shape (13, 2) with coordinates of points in the square.
    ...
    # Initial guess: Optimized initial guess using a Sobol sequence-like approach with jitter
    initial_guess = np.zeros(2 * N)
    for i in range(N):
        initial_guess[2 * i] = (i + np.random.uniform(-0.1, 0.1)) / N  # Evenly spread x-coordinates with jitter
        initial_guess[2 * i + 1] = ((i * np.sqrt(2)) % 1) + np.random.uniform(-0.1, 0.1)  # More sophisticated y-coordinate spread with jitter

    initial_guess = np.clip(initial_guess, 0.0, 1.0)

    # Define bounds: each coordinate must be within [0, 1]
    bounds = [(0.0, 1.0) for _ in range(2 * N)]

    # Optimization using SLSQP with refined parameters and increased restarts
    best_result = None
    best_discrepancy = float('inf')

    for _ in range(50):  # Multiple restarts to escape local minima - increased restarts even more
        result = minimize(
            discrepancy_wrapper,
            initial_guess,
            method='SLSQP',
            bounds=bounds,
            options={'maxiter': 25000, 'ftol': 1e-15, 'iprint': 0, 'disp': False}  # Increased iterations and tolerance
        )

        current_discrepancy = star_discrepancy(result.x.reshape(N, 2))

        if current_discrepancy < best_discrepancy:
            best_discrepancy = current_discrepancy
            best_result = result
        
        # Perturb the initial guess for the next restart, focusing on diversification
        perturbation = np.random.uniform(-0.05, 0.05, size=2 * N)
        initial_guess = np.clip(result.x + perturbation, 0.0, 1.0) if result is not None else np.clip(initial_guess + perturbation, 0.0, 1.0)
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=13"""
    A = construct_star()
    return A
