...
"""Finding optimal configuration of 60 points in a square that mimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function constructs 60 points that are optimized using scipy.optimize.minimize.
    Returns:
        A: np.array of shape (60, 2) with coordinates of points in the square.
        x and y coordinates are in the range [0, 1].
    """
    N = 60
    initial_points = np.zeros((N, 2))
    for i in range(N):
        initial_points[i, 0] = (i + 0.5) / N  # Evenly spaced x
        initial_points[i, 1] = ((i * 0.61803398875 + 0.5) % 1) # Golden ratio and offset y
    ...
    # Define bounds for each point to be within [0, 1]
    bounds = [(0.0, 1.0) for _ in range(2 * N)]

    # Use SLSQP optimizer with tighter tolerance and increased iterations
    # More aggressive optimization parameters.  Increase the number of iterations and the function tolerance.
    result = minimize(objective_function, initial_points_flat, method='SLSQP', bounds=bounds,
                     options={'maxiter': 30000, 'ftol': 1e-16, 'iprint': 0, 'eps': 1e-12}) # Increased iterations and tolerance, added eps

    # Reshape the optimized points
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=60"""
    A = construct_star()
    return A
    ...
    for i_point in range(N_arg): # Iterate through each point
        ...
        for k_dim in range(D_arg): # Iterate through each dimension for the current point
            if points_X_arg[i_point, k_dim] > y_corner_arg[k_dim]: # Point is outside this dimension
                ...
                break
            elif abs(points_X_arg[i_point, k_dim] - y_corner_arg[k_dim]) < 1e-15: # Point is on the line, using a tolerance
