...
"""Finding optimal configuration of 4 points in a square that minimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function uses scipy.optimize.minimize to find the optimal positions of 4 points.

    Returns:
        A: np.array of shape (4, 2) with coordinates of points in the square.
        x and y coordinates are in the range [0, 1].
    """

    def objective_function(x):
        points = x.reshape(4, 2)
        return star_discrepancy(points)

    # Initial guess for the point positions. Optimized initial guess based on symmetry.
    ...
    # Constraints to enforce ordering and avoid trivial solutions.
    constraints = (
        {'type': 'ineq', 'fun': lambda x: x[3] - x[1]},  # y4 > y2
        {'type': 'ineq', 'fun': lambda x: x[2] - x[0]},  # x3 > x1
        {'type': 'ineq', 'fun': lambda x: x[5] - x[7]}, # y2 > y3
        {'type': 'ineq', 'fun': lambda x: x[4] - x[6]} # x2 > x4
    ...
    result = minimize(objective_function, initial_guess, method='SLSQP', bounds=bounds,
                     ...
                     options={'maxiter': 50000, 'ftol': 1e-15, 'eps': 1e-15})

    # Reshape the optimized result back into a 4x2 array
    A = result.x.reshape(4, 2)
    return A
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=4"""
    A = construct_star()
    return A
