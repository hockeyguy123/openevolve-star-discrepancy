...
"""Finding optimal configuration of 180 points in a square that mimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function constructs 180 points that are evenly distributed.
    The points are represented in coordinates (x, y) in the square
    x and y are in the range [0, 1].
    Returns:
        A: np.array of shape (180, 2) with coordinates of points in the square.
        x and y coordinates are in the range [0, 1].
    """
    N = 180
    
    # Initial guess: Optimized stratified sampling with a carefully chosen offset.
    initial_points = np.zeros((N, 2))
    for i in range(N):
        initial_points[i, 0] = (i + 0.5) / N  # X coordinates: Stratified sampling with offset
        initial_points[i, 1] = ((i * 0.618034 + 0.5) % 1)  # Y coordinates: Golden ratio sequence with offset
    
    def objective_function(x):
        points = x.reshape(N, 2)
        return star_discrepancy(points)  # Minimize discrepancy directly
    ...
    # Optimization using SLSQP with increased iterations and refined options
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=180"""
    A = construct_star()
    return A
