...
"""Finding optimal configuration of 12 points in a square that mimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function constructs 12 points that are evenly distributed.
    The points are represented in coordinates (x, y) in the square
    x and y are in the range [0, 1].
    Returns:
        A: np.array of shape (12, 2) with coordinates of points in the square.
    ...
    # Initial guess: good lattice points
    initial_points = np.zeros((N, 2))
    initial_points[:, 0] = (np.arange(N) + 0.5) / N  # Centered initial points
    initial_points[:, 1] = (np.arange(N) * (np.sqrt(5) - 1) / 2) % 1  # Golden ratio sequence

    # Improved initial guess with jitter, but smaller jitter
    jitter_amount = 0.02
    initial_points += np.random.uniform(-jitter_amount, jitter_amount, size=(N, 2))
    ...
    # Increased maxiter, ftol, and use more restarts to escape local minima
    optimized_points = None
    best_discrepancy = float('inf')
    
    for _ in range(15):  # Increased restarts
        # Perturb initial guess for each restart, reducing the perturbation size over iterations
        if optimized_points is not None:
            perturbation_size = 0.003 * (0.9 ** _)  # Smaller perturbation
            initial_guess = optimized_points.flatten()
            initial_guess += np.random.normal(0, perturbation_size, size=2 * N)
            initial_guess = np.clip(initial_guess, 0.0, 1.0)
        else:
            initial_guess = initial_points.flatten()

        result = minimize(discrepancy_to_minimize, initial_guess, method='SLSQP', bounds=bounds, options={'maxiter': 15000, 'ftol': 1e-15}, tol=1e-15)
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=12"""
    A = construct_star()
    return A
