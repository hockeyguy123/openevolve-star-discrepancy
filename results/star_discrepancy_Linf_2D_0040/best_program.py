...
"""Finding optimal configuration of 40 points in a square that minimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function constructs 40 points.
    Returns:
        A: np.array of shape (40, 2) with coordinates of points in the square.
        x and y coordinates are in the range [0, 1].
    """
    N = 40
    
    # Improved initialization using a Sobol sequence-like approach
    initial_points = np.zeros((N, 2))
    for i in range(N):
        initial_points[i, 0] = (2 * i + 1) / (2 * N)
        initial_points[i, 1] = ((i * (np.sqrt(5) - 1) / 2 + 0.5) % 1) # Shifted for better distribution

    # Small jitter to break symmetry and avoid local minima
    jitter_std = 0.002  # Even smaller jitter for finer tuning
    initial_points += np.random.normal(0, jitter_std, size=(N, 2))
    ...
    # SLSQP optimizer with increased iterations, tighter tolerance, and gradient
    result = minimize(discrepancy_loss, x0, method='SLSQP', bounds=bounds,
                      options={'maxiter': 20000, 'ftol': 1e-18}) # Increased iterations and tighter tolerance
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=40"""
    A = construct_star()
    return A
...
def _calculate_single_box_discrepancy_numba(points_X_arg: np.ndarray, 
    ...
    for i_point in range(N_arg): # Iterate through each point
        ...
        # Optimized: Early exit if a point is outside the box
        ...
        if point_is_in_box:
            ...
            for k_dim in range(D_arg):
                if points_X_arg[i_point, k_dim] == y_corner_arg[k_dim]:
                    ...
                    break  # Only need to find one dimension where it's on the line
...
def star_discrepancy(points_X: np.ndarray) -> float:
    ...
    # Optimized: Generate grid lines only once and reuse
    grid_lines_per_dim = []
    for j in range(D):
    ...
    if not all(len(gl) > 0 for gl in grid_lines_per_dim):
        ...
        # Optimized: Use a pre-computed list of corners
        corner_list = list(itertools.product(*grid_lines_per_dim))
        for y_corner_tuple in corner_list:
