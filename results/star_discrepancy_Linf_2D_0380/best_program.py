...
"""Finding optimal configuration of 380 points in a square that minimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function constructs 380 points that are evenly distributed and then optimizes them.
    The points are represented in coordinates (x, y) in the square
    x and y are in the range [0, 1].
    Returns:
        A: np.array of shape (380, 2) with coordinates of points in the square.
        x and y coordinates are in the range [0, 1].
    """
    N = 380
    A = np.zeros((N, 2))

    # Optimized low-discrepancy sequence using a combination of sequences
    for i in range(N):
        # Improved x-coordinate distribution: more even spacing
        A[i, 0] = (i + 0.3) / N  # Centered within each interval, slight offset
        # Improved y-coordinate distribution: golden ratio sequence with better initial phase and offset
        A[i, 1] = ((i * 0.61803398875 + 0.1) % 1) # Experimentally determined offset
    ...
    initial_guess = A.flatten()
    bounds = [(0.0, 1.0) for _ in range(2 * N)]  # Ensure points stay within [0, 1]
    
    # Use SLSQP optimizer with tighter tolerances and constraints
    result = minimize(discrepancy_wrapper, initial_guess, method='SLSQP', bounds=bounds,
                      options={'maxiter': 50, 'ftol': 1e-5, 'iprint': 0})  # Reduced maxiter for faster execution

    if result.success:
        A = result.x.reshape(N, 2)
    else:
        print("Optimization failed, returning initial points.")
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=380"""
    A = construct_star()
    return A
