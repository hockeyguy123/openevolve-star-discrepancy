...
"""Finding optimal configuration of 18 points in a square that minimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function constructs 18 points that are evenly distributed.
    The points are represented in coordinates (x, y) in the square
    x and y are in the range [0, 1].
    Returns:
        A: np.array of shape (18, 2) with coordinates of points in the square.
        x and y coordinates are in the range [0, 1].
    """
    N = 18
    
    # Improved initial points using a Sobol sequence-like approach
    initial_points = np.zeros((N, 2))
    for i in range(N):
        initial_points[i, 0] = (2 * i + 1) / (2 * N)  # Centered within each interval
        initial_points[i, 1] = ((i * 0.61803398875) % 1)  # Golden ratio sequence
    ...
    # Use SLSQP optimizer with increased iterations and stricter tolerance
    # Employ a more robust initialization by trying multiple random starts
    num_restarts = 50  # Increased number of restarts
    best_result = None
    best_score = np.inf  # Initialize with a very high value

    for _ in range(num_restarts):
        # Perturb the initial guess with a small random offset, using a more targeted approach
        x0_perturbed = x0 + np.random.uniform(-0.0005, 0.0005, size=x0.shape)  # Slightly increased perturbation range
        x0_perturbed = np.clip(x0_perturbed, 0.0, 1.0)  # Ensure values stay within bounds

        result = minimize(objective_function, x0_perturbed, method='SLSQP', bounds=bounds, options={'maxiter': 6000, 'ftol': 1e-13, 'iprint': 0})  # Further increased iterations, tighter tolerance, suppressed output
    ...
    # Reshape the optimized result back into the points array
    A = best_result.x.reshape((N, 2))
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=18"""
    A = construct_star()
    return A
