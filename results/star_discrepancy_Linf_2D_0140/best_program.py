...
"""Finding optimal configuration of 140 points in a square that mimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function constructs 140 points that are evenly distributed.
    The points are represented in coordinates (x, y) in the square
    x and y are in the range [0, 1].
    Returns:
        A: np.array of shape (140, 2) with coordinates of points in the square.
        x and y coordinates are in the range [0, 1].
    """
    N = 140
    
    # Improved initial guess using a combination of Sobol and Hammersley
    A = np.zeros((N, 2))
    for i in range(N):
        A[i, 0] = (i + 0.5) / N  # Hammersley x-coordinate
        A[i, 1] = ((i * 0.61803398875 + 0.5) % 1)  # Golden ratio conjugate for y-coordinate (more robust)
    
    # Define the objective function (negative of the score, since we minimize)
    def objective(x):
        points = x.reshape(N, 2)
        return -score_star(points)

    # Constraints: 0 <= x_i, y_i <= 1
    bounds = [(0.0, 1.0) for _ in range(2 * N)]
    
    # Optimization using SLSQP with refined options for better convergence
    initial_guess = A.flatten()
    result = minimize(objective, initial_guess, method='SLSQP', bounds=bounds, 
                    options={'maxiter': 25000, 'ftol': 1e-13, 'iprint': 0})  # Increased maxiter, tighter ftol, suppressed output

    # Reshape the optimized result back to the (N, 2) array
    A_optimized = result.x.reshape(N, 2)
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=140"""
    A = construct_star()
    return A
