...
"""Finding optimal configuration of 660 points in a square that mimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function constructs 660 points.
    The points are represented in coordinates (x, y) in the square
    x and y are in the range [0, 1].
    Returns:
        A: np.array of shape (660, 2) with coordinates of points in the square.
        x and y coordinates are in the range [0, 1].
    """
    N = 660
    A = np.zeros((N, 2))

    # Optimized distribution using a combination of stratified sampling and a shifted golden ratio sequence
    golden_ratio = 0.618033988749895

    # Vectorized x-coordinate calculation with optimized stratification
    A[:, 0] = (np.arange(N) + 0.5 + (np.arange(N) % 4) * 0.125) / N

    # Vectorized y-coordinate calculation using golden ratio and tent map
    y = (np.arange(N) * golden_ratio) % 1.0
    A[:, 1] = np.where(y < 0.5, 2 * y, 2 * (1 - y))
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=660"""
    A = construct_star()
    return A
