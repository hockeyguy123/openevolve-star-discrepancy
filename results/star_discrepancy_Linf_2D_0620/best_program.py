...
"""Finding optimal configuration of 620 points in a square that mimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function constructs 620 points that are evenly distributed.
    The points are represented in coordinates (x, y) in the square
    x and y are in the range [0, 1].
    Returns:
        A: np.array of shape (620, 2) with coordinates of points in the square.
        x and y coordinates are in the range [0, 1].
    """
    N = 620
    A = np.zeros((N, 2))
    
    # Optimized stratified sampling with a fixed offset for better distribution
    x = (np.arange(N) + 0.5) / N  # Centered stratified sampling

    # Improved golden ratio sequence with optimized starting point and scaling
    golden_ratio = 0.6180339887498949
    y = ((np.arange(N) * golden_ratio) % 1) #  golden ratio, remove offset

    # Optimized perturbation with a smaller magnitude and a different distribution
    rng = np.random.default_rng(seed=42)
    perturbation = rng.uniform(-0.015 / N, 0.015 / N, size=N)  # Uniform perturbation
    y = np.clip(y + perturbation, 0, 1)  # Clip to ensure values stay within [0, 1]

    # Optimized tent map transformation using vectorization (Slightly adjusted parameters)
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=620"""
    A = construct_star()
    return A
