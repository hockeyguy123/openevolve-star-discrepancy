...
def construct_star() -> np.ndarray:
    """
    Constructs 7 points on a unit square [0, 1] x [0, 1] to minimize L-infinity star discrepancy.
    This function uses scipy.optimize.minimize to fine-tune the point positions.

    Returns:
        A: np.array of shape (7, 2) with coordinates of points in the square.
        x and y coordinates are in the range [0, 1].
    """

    def discrepancy_wrapper(x):
        points = x.reshape(7, 2)
        return star_discrepancy(points)

    # Initial guess for point positions (refined) - using a more spread out initial guess
    ...
    bounds = [(0.0, 1.0)] * 14  # 7 points * 2 coordinates

    # Use SLSQP optimizer with increased maxiter and ftol, and more restarts
    best_result = None
    best_discrepancy = float('inf')

    for _ in range(70):  # Multiple restarts, increased from 60 to 70
        # Perturb initial guess for the next restart, using a smaller perturbation
        initial_guess_perturbed = np.clip(initial_guess + 0.0004 * np.random.randn(14), 0.0, 1.0) # Even smaller perturbation and clipping, reduced perturbation magnitude even further
        result = minimize(discrepancy_wrapper, initial_guess_perturbed, method='SLSQP', bounds=bounds, options={'maxiter': 70000, 'ftol': 1e-23}) # Increased maxiter and ftol even more
        
        if result.success and result.fun < best_discrepancy:
            best_discrepancy = result.fun
            best_result = result

    # Reshape the optimized result into the desired (7, 2) array
    if best_result is not None:
        A = best_result.x.reshape(7, 2)
    else:
        A = initial_guess.reshape(7, 2) # Fallback to initial guess if no successful optimization
        A = np.clip(A, 0.0, 1.0)  # Ensure points are within bounds
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=7"""
    A = construct_star()
    return A
