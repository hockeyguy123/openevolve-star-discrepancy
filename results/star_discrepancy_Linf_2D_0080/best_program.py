...
"""Finding optimal configuration of 80 points in a square that mimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function constructs 80 points that are evenly distributed.
    The points are represented in coordinates (x, y) in the square
    x and y are in the range [0, 1].
    Returns:
        A: np.array of shape (80, 2) with coordinates of points in the square.
    ...
    # Initial guess: Centered linear spacing and improved Golden Ratio variant
    initial_points = np.zeros((N, 2))
    golden_ratio = (1 + np.sqrt(5)) / 2
    phi = (np.sqrt(5) - 1) / 2  # Use the golden ratio conjugate
    for i in range(N):
        initial_points[i, 0] = ((i+0.5) / N) % 1 # Add small offset
        initial_points[i, 1] = (((i+0.5) * phi) % 1) % 1 # Add small offset
    initial_guess = initial_points.flatten()

    # Define bounds for the optimization
    bounds = [(0.0, 1.0) for _ in range(2 * N)]

    # Optimization using SLSQP with refined options and callback
    def callback(xk):
        # Optional: Implement some logic to print progress or save intermediate results
        pass
    
    result = minimize(discrepancy_wrapper, initial_guess, method='SLSQP', bounds=bounds,
                      options={'maxiter': 5000, 'ftol': 1e-12, 'iprint': 0}, callback=callback)

    # Reshape the optimized result
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=80"""
    A = construct_star()
    return A
