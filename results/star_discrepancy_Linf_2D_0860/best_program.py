...
"""Finding optimal configuration of 860 points in a square that mimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function constructs 860 points that are evenly distributed.

    Returns:
        A: np.array of shape (860, 2) with coordinates of points in the square.
        x and y coordinates are in the range [0, 1].
    """
    N = 860
    A = np.zeros((N, 2))

    # Optimized distribution using a combination of stratified sampling and a sinusoidal offset
    for i in range(N):
        # Stratified sampling in x
        A[i, 0] = (i + 0.5) / N

        # Improved y-coordinate distribution using a linear congruential generator (LCG)
        # and a tent map-like transformation for better uniformity.
        # Combining LCG with tent map characteristics.
        lcg_val = (i * 0.618034 + 0.5) % 1.0  # Golden ratio based LCG with different multiplier
        if lcg_val < 0.5:
            A[i, 1] = 2 * lcg_val
        else:
            A[i, 1] = 2 * (1 - lcg_val)
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=860"""
    A = construct_star()
    return A
