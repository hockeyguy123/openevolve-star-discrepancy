...
"""Finding optimal configuration of 1020 points in a square that minimizes the L-infinity star discrepancy."""
...
def construct_star() -> np.ndarray:
    ...
    This function constructs 1020 points that are evenly distributed.
    The points are represented in coordinates (x, y) in the square
    x and y are in the range [0, 1].
    Returns:
        A: np.array of shape (1020, 2) with coordinates of points in the square.
    ...
    # 1. Stratified sampling in x-direction
    # 2. Optimized shifted Halton sequence in y-direction to reduce discrepancy
    
    # Using vectorization for faster computation
    x_coords = (np.arange(N) + 0.5) / N
    y_coords = ((np.arange(N) * golden_ratio) % 1)

    # Refinement: Apply a tent transformation to y_coords to improve uniformity
    y_coords = np.where(y_coords < 0.5, 2 * y_coords, 2 * (1 - y_coords))

    A[:, 0] = x_coords  # Stratified sampling for x-coordinates
    A[:, 1] = y_coords # Adjusted golden ratio sequence for y-coordinates with shift
...
def run_star() -> np.ndarray:
    """Run the star constructor for n=1020"""
    A = construct_star()
    return A
